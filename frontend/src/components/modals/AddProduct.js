...
const AddProduct = () => {
  ...
  const fetchBrandsByCategory = (categoryId) => {
    fetch(`http://localhost:4000/api/v1/get-brands-by-category/${categoryId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicateProduct = allProduct.find((product) => product.name.toLowerCase() === data.name.trim().toLowerCase());
    if (duplicateProduct) {
